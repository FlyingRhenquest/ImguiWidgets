...
namespace fr::Imgui {
  ...
  public:
    ...
    using PtrType = std::shared_ptr<RequirementWindow>;
    using Parent = CommitableNodeWindow;

    RequirementWindow(std::string title = "Requirement") : Parent(title) {
      _functional = false;
    ...
    void begin() override {
      ...
      if (node) {
        ...
        if (!_editable || node->isCommitted()) {
          ImGui::BeginTooltip();
          ImGui::Text("Node is not editable or is committed; this value will not change.");
          ImGui::EndTooltip();
        }
        ImGui::Text("Title: ");
        ImGui::SameLine();
